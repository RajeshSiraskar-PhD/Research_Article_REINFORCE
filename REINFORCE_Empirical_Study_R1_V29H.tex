...
\begin{document}
	...
	Application of RL for PdM has been surveyed by \cite{Panzer2021, Erhan2021, siraskar2023}. \hlc{There are no research articles that cover AutoRL for predictive maintenance. We therefore widened our scope and searched for AutoML for PdM resulting in 21 articles (of which were 14 were conference papers and 7 piblished in journals), Fig. {\ref{fig_artcilestats}}, however the research coverage remained limited to supervised machine learning and excluded RL and AutoRL.}\footnote{As of: 21-Dec-2024, across IEEE Xplore\texttrademark{}, Scopus\texttrademark{} and Web Of Science\texttrademark{}}.
	...
	\begin{figure}[hbt!]
		...
		\caption{Research articles related to AutoML and PdM. None were found on AutoRL for PdM.}
	...
	\begin{table}[hbtp!]
		...
		{\begin{tabular}[h]{L{2cm} L{7cm} L{3.5cm}}
				...
				\hlc{Deduction} & \hlc{Design and build an experimentation set-up. Design a set of experiments to test hypothesis}.&Implementation Sec. \ref{sec_Hypothesistesting}\\
	...
	\hlc{RL algorithms are known to be extremely sensitive to hyperparameter and network architecture settings,} \cite{henderson2018deep, andrychowicz2021matters}. \hlc{We conduct a basic hyperparameter interaction and sensitivity analysis in this section. To select the best hyperparameters to conduct the sensitivity analysis we referred to recent research} \cite{autorl:shala2022} and \cite{Eimer2023AutoRL}. \cite{autorl:shala2022} \hlc{created the ``AutoRL-Bench 1.0'' for benchmarking AutoRL performance. We selected the {\textit{same}} common hyperparameters they studied across all RL algorithms they benchmark i.e. learning rate and the discount factor $\gamma$.} \cite{Eimer2023AutoRL} \hlc{mention how some often overlooked hyperparameters, such as the discount factor, significantly impacts the algorithm`s performance. In addition to these suggestions, we selected the network activation function for analysis, thus covering network architecture as well.}
	
	\hlc{For our analysis we will train the model and compute the average episodic rewards, over 10 rounds of training. Fig. {\ref{fig_SA_LR_Relu}} shows the impact of changing learning rates on the average episodic rewards, for a particular $\gamma$ value, with the network activation function set to ReLU. Similarly Fig. {\ref{fig_SA_LR_Tanh}} shows the impact when the activation function is changed to Tanh. We observe that the optimal settings are the higher learning rates $5\times10^{-3} - 1\times10^{-2}$ and highest $\gamma$ value $0.99$, using the ReLU activation function.}
	
	\hlc{Hyperparameter settings:}
	\begin{enumerate}    
	...
	\textbf{\hlc{Strategies for reducing training time}}: \cite{Lavet2016} \hlc{has conducted an extensive analysis on the interaction of $\gamma$ and the learning-rate. Lower values of {$\gamma$} lead to faster convergence but by the very definition of {$\gamma$}, often result in myopic policies. One strategy for speeding up the learning process is to increase the discounting rate over time, rather than keeping it static.} \cite{Lavet2016} \hlc{further showed that simultaneously decreasing the learning rate while increasing {$\gamma$} further reduces training time and thus improves learning speed. Our study points to one simple yet effective strategy of using ReLU activation layer for reducing training time, without sacrificing performance, Fig. } \ref{fig_perfcost}.
	...
	\hlc{As a final point, we compare RL to the simpler ML methods used for a similar problem, see Table {\ref{tbl_ml}}. Supervised machine learning requires labeled data, RL on the other hand determines the right action via trial and error. The manufacturing industry often lacks labeled fault data as reported by extremely recent surveys,} \cite{Leite2025, Hexagon2024}, \hlc{and this is one good reason for promoting RL. As can be seen in the existing literature, supervised ML, when labeled fault data is available will surely perform better, demands far lower compute resources and has very stable methods. Unlike the closed-loop RL, supervised ML is open-looped and the impact of hyperparameters is very far more ascertainable and predictable,}  \cite{autorl:parker2022}.
