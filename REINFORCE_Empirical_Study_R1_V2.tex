...
%% 19-Dec-2024: Highlights
%% 28-Dec-2024: Begin comments
%% 28-Dec-2024: V2 - Methodology overhaul 
...
\begin{document}
	...
	%% $$$ R2-9	
	\hlc{This empirical study was undertaken after seeing preliminary encouraging results of the simple implementation of REINFORCE against the more advanced algorithms. Our study broadly follows the methodology outlined in}  \cite{Bouchrika2024, Emerald2021}, \hlc{ and is based on the original \textit{empirical cycle} proposed by Adriaan D De Groot} \cite{DeGroot1969}.
	
	\hlc{Using an exploratory research approach to arrive at a hypothesis} \cite{Politzer2021}, \hlc{we built an experimentation set-up, collected data, systematically analyzed it and tabled our observations and using abductive reasoning present some reasons for the behavior we observed.}	
	...
	\textbf{A.D. de Groots empirical cycle}:[3]
	\begin{enumerate}
		\item Observation: REINFORCE performs better on the predictive maintenance  problem.
		\item Induction: Formulate a high-level hypothesis.			
		\item Deduction: Design a set of experiments to test this hypothesis.
		\item Testing: Conduct the experiments and collect data.
		\item Evaluation: Evaluate the data and present the results of the experiments with interpretation based on abductive reasoning, that is the most reasonable explanation for the observation.
	\end{enumerate}
	...
	%% $$$ R1-3
	\hlc{For a classification problem, the choice of precision versus recall will depend on the industry and application}. Timely replacements, $TP$, ensure work piece quality \hlc{while still maintaining minimal production downtime. Precision is driven by lower $FP$s i.e. reducing unnecessary replacements}. While we prefer a high precision, we do want a reasonably high recall \hlc{such as in several critical industrial environments, a higher recall is preferred where failing to replace the tool can lead to serious consequences, demanding lower $FN$s.} \hlc{Depending on the industrial application one can use a \textit{weighted} F1-score equation} (\ref{eq_Fbeta}) \hlc{that is oriented toward precision or recall}. For high precision applications set a $\beta < 1.0$; \hlc{for applications demanding higher recall, set a $\beta > 1.0$.} For \hlc{this research} we set $\beta$ to $0.5$ and \hlc{suggest setting $\beta$ to $2.0$ for applications where recall is critical}.
...
%	\end{table}
	...
	Our final set of results are related to the training time required for each algorithm. The na\"ive REINFORCE algorithm is extremely slow, with a very high variance in training time, as seen in Fig. \ref{fig_tr-time}.
	\begin{figure}[hbt!]
		\centering
	...
	\subsection{\hlc{Sensitivity analysis of hyperparameters on the REINFORCE algorithm}}
		
	\hlc{RL algorithms are known to be extremely sensitive to hyperparameter and network architecture settings,} \cite{henderson2018deep, andrychowicz2021matters}. \hlc{Recent research on how RL hyperparameters should be tuned } \cite{Eimer2023AutoRL}, \hlc{is a significant contribution to the RL field.
		
	We analyzed the sensitivity of REINFORCE to hyperparameters via experiments. As a simple algorithm REINFORCE has fewer hyperparameters compared to the more advanced algorithms and we selected learning rate, the discount factor $\gamma$ and the network activation functions for study}. 
