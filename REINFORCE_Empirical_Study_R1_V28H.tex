...
\begin{document}
	...
	\hlc{RL algorithms are known to be extremely sensitive to hyperparameter and network architecture settings,} \cite{henderson2018deep, andrychowicz2021matters}. \hlc{To select the best hyperparameters to conduct the sensitivity analysis we referred to recent research} \cite{autorl:shala2022} and \cite{Eimer2023AutoRL}. \cite{autorl:shala2022} \hlc{created the ``AutoRL-Bench 1.0'' for benchmarking AutoRL performance. We selected the {\textit{same}} common hyperparameters they studied across all RL algorithms they benchmark i.e. learning rate and the discount factor $\gamma$.} \cite{Eimer2023AutoRL} \hlc{mention how some often overlooked hyperparameters, such as the discount factor, significantly impacts the algorithm`s performance. In addition to these suggestions, we selected the network activation function for analysis, thus covering network architecture as well.}
	...
	\subsubsection*{\hlc{The cost of performance improvement}}
