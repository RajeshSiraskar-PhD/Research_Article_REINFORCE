...
\begin{document}
	...
	\hlc{All AutoRL related work} \cite{autorl:afshar2022, autorl:parker2022, autorl:shala2022, Eimer2023AutoRL}, \hlc{point unanimously toward three research gaps: First, unlike AutoML pipelines, AutoRL as a usable framework has not yet been developed, second there is no ideal hyperparameter configuration setup that is clearly optimal for a set environments and third domain specific work is required for AutoRL to be effective.} 
	
	%% LR consolidate
	\hlc{The authoritative survey by} \cite{autorl:parker2022}\hlc{mention that as a completely closed-loop system, in RL, each of its component influences the other. This therefore poses a challenge for automatic tuning as any interim system evaluation (during training) is highly likely to be stochastic due to the interaction of various noise sources (e.g. policy, environment). They concluded that no AutoRL approach has been developed to offer the best combination of an algorithm itself and then its configuration}. 
...
%		
%		===================
%		shala2022 SURVEY SHOWS =--=== >> Still, HPO and AutoRL methods have not yet found widespread adoption by the RL community as methods are often not extensively evaluated
%		versa. Contrary to
%		practitioners’ hopes, there exists no silver bullet hyper-parameter configuration that is clearly optimal
%		in the vast majority of the environments. The take-home message is that optimal hyperparameters are
%		environment-specific and must be carefully tuned.
%				===================
%		
%		
%		
...
%	%% $$$ R2-4 Novelty
%	
%	\hlc{Our research validates this statement, for REINFORCE, by addressing three research gaps -- first, a detailed statistically backed study of the computationally simple REINFORCE, second, studying the untuned behaviour of industry standard algorithms and finally as Eimer et al. {\cite{Eimer2023AutoRL}} highlight the importance of studying AutoRL, for RL's domain specific behaviour, in ``as-of-yet unexplored domains''.} 
%	
%	We studied untuned algorithms as available AutoML systems tend to be highly configurable \cite{Tornede2020AutoML} - resulting in overwhelming practitioners. In the same study \cite{Tornede2020AutoML} mentions the lack of AutoML for time-series based data .
%	- Issue with standard algos: Timeseries RUL prob has varying length of time - so converts to fixed length"
%	Say in text \cite{ParkerHolder2022AutoRL} Thus, hyperparameter settings
%	might require very different values over the whole training run. -- The most authoratiative survey \footnote{Researchers from Universities - Oxford, Leibniz, Freiburg and from Google, Meta and Amazon}. 
...
%	"parker-holder - (1) RL is a complete closed-loop system. As such, it is likely each of the components
%	discussed has an influence on others, 
%	(2) The challenge is compounded in RL since evaluations in RL are almost always necessarily stochastic and much more noisy than in supervised learning, due to various sources (e.g. policy, environment), which can be a challenge for any form of automatic tuning."
%	
%	
%	
%	%% ##################
%	%% GAPS
%	%% ##################
%	1. specific to an environment \cite{autorl:parker2022} and \cite{Eimer2023AutoRL} + Nikolai Matni 2019: It is difficult to say which approach is better without selecting a specific problem to which to apply them
%	
%	2. \cite{autorl:shala2022} -- three points
%		2.1.  -- did not study REINFORCE only - PPO, DDPG, A2C, SAC and TD3 
%		2.2. and NOT other env. just , across 22 OpenGym, environments. 
%		2.3 two common across all five algorithms -- the learning rate and discount factor $\gamma$
%	3. all three agree on xxx	
